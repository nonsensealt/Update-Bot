...
async function checkUpdate() {
  try {
    ...
    if (!response.ok) {
      console.log(`Roblox API request failed: ${response.status}`);
      return;
    }
    ...
    if (!lastUpdate || currentUpdate > lastUpdate) {
      ...
      const thumbResponse = await fetch(`https://thumbnails.roblox.com/v1/games/icons?universeIds=${UNIVERSE_ID}&returnPolicy=PlaceHolder&size=512x512&format=Png&isCircular=false`);
      let imageUrl = 'https://via.placeholder.com/512x512.png';
      if (thumbResponse.ok) {
        const thumbData = await thumbResponse.json();
        imageUrl = thumbData?.data?.[0]?.imageUrl || imageUrl;
      } else {
        console.log(`Thumbnail request failed: ${thumbResponse.status}`);
      }
      ...
      payload.embeds = [{
        ...
        footer: {
          text: 'Roblox Update Bot - Made By @nonsense'
        }
      }];
